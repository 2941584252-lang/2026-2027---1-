...
export const Navbar: React.FC = () => {
  ...
  const desktopLinkClass = (active: boolean, highlight?: boolean) =>
    `flex items-center gap-1.5 px-3 py-2 text-[13px] font-medium whitespace-nowrap rounded-lg transition-all ${
      active
        ? highlight
  ...
  return (
    <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/80 dark:bg-slate-900/80 border-b border-slate-200/80 dark:border-slate-800 transition-colors">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-3 group shrink-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-xl font-bold tracking-tight whitespace-nowrap bg-gradient-to-r from-brand-600 via-indigo-600 to-accent-600 bg-clip-text text-transparent dark:from-brand-400 dark:to-accent-400">
                  智汇工坊
                </span>
                <span className="text-[10px] font-semibold tracking-wider uppercase px-1.5 py-0.5 rounded-full bg-brand-50 text-brand-700 dark:bg-brand-950/80 dark:text-brand-300 border border-brand-200/60 dark:border-brand-800/60 whitespace-nowrap">
                  AI 共创平台
                </span>
              </div>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 hidden 2xl:block whitespace-nowrap">
          ...
          <nav className="hidden xl:flex items-center gap-1.5">
            {primaryItems.map((item) => {
              ...
              return (
                <Link key={item.path} to={item.path} className={desktopLinkClass(active, item.highlight)}>
            ...
            <div className="relative" ref={moreRef}>
              <button
                ...
                className={`${desktopLinkClass(moreActive)} ${moreOpen ? 'bg-slate-100/70 dark:bg-slate-800/60 text-slate-900 dark:text-white' : ''}`}
              >
                <span>更多</span>
              ...
              {moreOpen && (
                <div className="absolute right-0 top-full mt-2 w-44 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl shadow-slate-900/10 py-1.5 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                  {moreItems.map((item) => {
                    const Icon = item.icon;
                    const active = isActive(item.path);
                    return (
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              <kbd className="hidden 2xl:inline-block text-[10px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 rounded text-slate-400">
                ⌘K
              </kbd>
            </button>
            ...
            <div className="hidden md:flex items-center gap-2 pl-2 border-l border-slate-200 dark:border-slate-800">
              ...
              <div className="text-left">
                <p className="text-xs font-semibold leading-tight text-slate-800 dark:text-slate-200 whitespace-nowrap">计创2502</p>
                <p className="text-[10px] text-brand-600 dark:text-brand-400 whitespace-nowrap">共创团队</p>
              </div>
            </div>
            ...
            <button
              ...
              className="p-2 xl:hidden rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
            >
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="xl:hidden border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-4 pt-2 pb-4 space-y-1 animate-in slide-in-from-top-2 duration-200">
